...
class ClientOffer {
  ...
  factory ClientOffer.fromJson(Map<String, dynamic> json) {
    ...
    final applicableRaw =
        json['applicableServices'] ?? json['applicable_services'];
    return ClientOffer(
      ...
      discountType:
          ...
          'percentage',
      discountValue:
          (json['discountValue'] ?? json['discount_value'] ?? 0) as int,
  ...
  String get discountBadge {
    if (isPermanent &&
        discountType == 'fixed' &&
        title.toLowerCase().contains('carte cadeau')) {
      return 'Dès $discountValue€';
    }
