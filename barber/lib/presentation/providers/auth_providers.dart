...
class AuthController extends StateNotifier<AuthState> {
  ...
  Future<void> bootstrapSession() async {
    ...
    if (appUser != null) {
      state = AuthState(
        ...
        user: reservationSession == null
            ? appUser
            : _mergeReservationProfile(appUser, reservationSession.user),
      );
  ...
  Future<void> login({
    ...
    try {
      ...
      state = AuthState(
        status: AuthStatus.authenticated,
        user: _mergeReservationProfile(
          loginResult.response.user,
          loginResult.reservationSession.user,
        ),
      );
      await _registerPushTokenBestEffort();
    } catch (e) {
  ...
  Future<void> register({
    ...
    try {
      // 1. ALWAYS register in the Main App DB first.
      // This enforces uniqueness and is the true source of truth for passwords.
      ...
      // Sync the new user to the Reservation System
      try {
        ...
        state = AuthState(
          ...
          user: _mergeReservationProfile(
            response.user,
            reservationSession.user,
          ),
        );
      } catch (_) {
        // If reservation sync fails, the user is still safely created in the app
        state = AuthState(
          status: AuthStatus.authenticated,
          user: response.user,
        );
      ...
      // 3. If the Main DB rejects the signup (e.g. Email exists), STOP completely.
      await _rollbackAppRegistration(password: password);
      await _ref.read(reservationSessionProvider.notifier).clearSession();

      String errorMessage = 'Une erreur est survenue lors de l\'inscription.';

      // This will pull the exact French message from your api_error.dart
  ...
  Future<void> _syncAppSessionBestEffort({
    ...
    try {
      ...
      state = AuthState(
        status: AuthStatus.authenticated,
        user: _mergeReservationProfile(response.user, reservationSession.user),
      );
      await _registerPushTokenBestEffort();
      return;
  ...
  User _mergeReservationProfile(
    User appUser,
    ReservationClientProfile profile,
  ) {
    final fullName = profile.fullName.trim();
